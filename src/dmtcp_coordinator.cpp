...
static const char* theHelpMessage =
  "COMMANDS:\n"
  "      This is updated file, If you see this, it means this has compiled and executed..\n"
...
/*****************************************************
This is where I add sending interests to checkpoints
******************************************************/
/*for (size_t i = 0; i < clients.size(); i++) {                                     // This for loop is for the interests being sent
      JASSERT_STDERR <<"Sending Checkpoint command Interest to " 
		     <<"ccnx://" << clients[i]->hostname()              // changed to add ccnx uri to the name --nitin
...
/*****************************************************
This is where I add receiving data from checkpoints
******************************************************/
/*for (size_t i = 0; i < clients.size(); i++) {                                     // This for loop is for the data being received
      JASSERT_STDERR <<"Receiving Null data as acknowledgement from " 
		     <<"ccnx://" << clients[i]->hostname()              // changed to add ccnx uri to the name --nitin
#ifdef PRINT_REMOTE_IP
        << "(" << clients[i]->ip() << ")"
#endif
...
bool dmtcp::DmtcpCoordinator::startCheckpoint()
{
       ...
       && !workersRunningAndSuspendMsgSent )
  {
    ...
    /******************************************************
    CCN Code for sending checkpoint interest to all nodes
    *******************************************************/
   /* int res = 0;
   
    //allocate memory for URI
    const char *URI = (char *) malloc(20);

    for (size_t i = 0; i < clients.size(); i++) {        // This for loop is for the data being received

    	 char *pre = new char[10];
    	 strcpy(pre, "ccnx://");
    	 const char *temp = clients[i]->hostname().c_str();   //append ccnx:/ with each host name
    	 ...
    	 //sprintf(temp1, "%s%s/c", pre, temp);     //the checkpoint request will look like ccnx://Van/c

    	 temp1=strcat(strcat(pre,temp),"/c"); 

    	 URI = temp1;       //put together the name of URI to which the interest is to be sent

    	 JASSERT_STDERR <<"Sent Checkpoint command Interest to " 
		     <<URI <<'/n';
}*/
   ...
   int res = 0;
    ...
    //allocate buffer for response (although we are not handling any response as none will be returned)
    ...
    for (size_t i = 0; i < clients.size(); i++) {        // This for loop is for the data being received
    	 ...
    	 URI = temp1;       //put together the name of URI to which the interest is to be sent
		    ...
		    //express interest
		    res = ccn_get(ccn, ccnb, NULL, timeout_ms, resultbuf, &pcobuf, NULL, 0);  //send checkpoint interests to all the hosts
...
int main ( int argc, char** argv )
{
  ...
  **********************************************/ 
  ...
  int res=0;  // res is for CCN only, no relation to dmtcp as such 
